...
public class AuthorizationConsentController {
    ...
    public String getMethodName(
        ...
        /*
         * Verificar se o usuario ja autorizou o consentimento anteriormente
         */
        OAuth2AuthorizationConsent consent = this.consentService.findById(client.getId(), principal.getName());

        /*
         * Processar a lista de scopos
         */
        String[] scopeArray = StringUtils.delimitedListToStringArray(scope, " ");
        Set<String> scopesParaAprovar = new HashSet<>(Set.of(scopeArray));

        Set<String> scopesAprovadosAnteriomente;

        if (consent != null) {
            scopesAprovadosAnteriomente = consent.getScopes();
            scopesParaAprovar.removeAll(scopesAprovadosAnteriomente);
        }else{
            scopesAprovadosAnteriomente = Collections.emptySet();
        }
        model.addAttribute("clientId", clientId);
        model.addAttribute("state", state);
        model.addAttribute("principalName", principal.getName());
        model.addAttribute("scopesParaAprovar", scopesParaAprovar);
        model.addAttribute("scopesAprovadosAnteriomente", scopesAprovadosAnteriomente);
